@php
    ...
    if ($installmentPlansState === []) {
        $installmentPlansState = [[
            ...
            'down_payment' => 0,
            'financing_fee' => 0,
            'interval_type' => \App\Services\InstallmentCalculatorService::INTERVAL_MONTHLY,
            'is_active' => true,
        ]];
    }
@endphp
...
<div class="admin-grid admin-grid-4">
    ...
    <div class="admin-card admin-kpi">
        ...
        <span class="admin-kpi__meta">Product-level plans</span>
    </div>
</div>
...
<div class="admin-card">
    <div class="admin-card__header">
        <div>
            ...
            <p class="admin-card__copy">Configure product-wide plans or target a single variant. Preview schedules are generated by the backend calculator.</p>
        </div>
        <button type="button" class="admin-button admin-button--secondary" id="add-plan">Add Plan</button>
    </div>
...
@push('scripts')
    <script>
        (() => {
            ...
            const installmentIntervalOptions = @json($installmentIntervalOptions);
            ...
            const buildPlans = () => {
                ...
                roots.plans.innerHTML = state.plans.map((plan, index) => `
                    <div class="product-form-card">
                        <input type="hidden" name="installment_plans[${index}][id]" value="${plan.id || ''}">
                        <input type="hidden" name="installment_plans[${index}][variant_key]" value="${escapeHtml(plan.variant_key || '')}">
                        <input type="hidden" name="installment_plans[${index}][product_variant_id]" value="${escapeHtml(plan.product_variant_id || '')}">
                        <div class="admin-grid admin-grid-2">
                            <div class="admin-field">
                                <label class="admin-label">Scope</label>
                                <select class="admin-select" name="installment_plans[${index}][scope]" data-plan-scope="${index}">
                                    <option value="product" ${plan.scope === 'product' ? 'selected' : ''}>All variants</option>
                                    <option value="variant" ${plan.scope === 'variant' ? 'selected' : ''}>Specific variant</option>
                                </select>
                            </div>
                            <div class="admin-field">
                                <label class="admin-label">Specific Variant</label>
                                <select class="admin-select" name="installment_plans[${index}][variant_selector]" data-plan-variant="${index}" ${plan.scope === 'variant' ? '' : 'disabled'}>
                                    <option value="">Select variant</option>
                                    ${state.variants.map((variant) => {
                                        const variantKey = escapeHtml(variant.client_key || '');
                                        const selected = variant.client_key === plan.variant_key || String(variant.id || '') === String(plan.product_variant_id || '') ? 'selected' : '';

                                        return `<option value="${variantKey}" ${selected}>${escapeHtml(variantLabel(variant))} (${escapeHtml(variant.sku || 'No SKU')})</option>`;
                                    }).join('')}
                                </select>
                                ${state.variants.length === 0 ? '<div class="admin-help">Generate variants before assigning a variant-specific plan.</div>' : ''}
                            </div>
                            <div class="admin-field">
                                <label class="admin-label">Number of Payments</label>
                                <input class="admin-input" type="number" min="2" name="installment_plans[${index}][months]" value="${escapeHtml(plan.months ?? 3)}" required>
                            </div>
                            <div class="admin-field">
                                <label class="admin-label">Down Payment</label>
                                <input class="admin-input" type="number" step="0.01" min="0" name="installment_plans[${index}][down_payment]" value="${escapeHtml(plan.down_payment ?? 0)}">
                            </div>
                            <div class="admin-field">
                                <label class="admin-label">Financing Fee</label>
                                <input class="admin-input" type="number" step="0.01" min="0" name="installment_plans[${index}][financing_fee]" value="${escapeHtml(plan.financing_fee ?? 0)}">
                            </div>
                            <div class="admin-field">
                                <label class="admin-label">Payment Interval</label>
                                <select class="admin-select" name="installment_plans[${index}][interval_type]">
                                    ${Object.entries(installmentIntervalOptions).map(([value, label]) => `
                                        <option value="${escapeHtml(value)}" ${plan.interval_type === value ? 'selected' : ''}>${escapeHtml(label)}</option>
                                    `).join('')}
                                </select>
                            </div>
                        </div>
                        <div class="admin-inline" style="justify-content:space-between; margin-top:12px; align-items:center;">
                            <span class="plan-scope-pill">${plan.scope === 'variant' ? 'Variant-specific override' : 'Product-level plan'}</span>
                            <button type="button" class="admin-link" data-preview-plan="${index}">Preview Schedule</button>
                        </div>
                        <div class="admin-inline" style="justify-content:space-between; margin-top:12px;">
                            <label style="display:flex; gap:10px; align-items:center;">
                                <input type="hidden" name="installment_plans[${index}][is_active]" value="0">
                                <input type="checkbox" name="installment_plans[${index}][is_active]" value="1" ${plan.is_active === false ? '' : 'checked'}>
                                <span>Active plan</span>
                            </label>
                            <div class="admin-inline">
                                <button type="button" class="admin-link" data-duplicate-plan="${index}">Duplicate</button>
                                <button type="button" class="admin-link" data-remove-plan="${index}">Remove</button>
                            </div>
                        </div>
                    </div>
                `).join('');

                roots.plans.querySelectorAll('[data-plan-scope]').forEach((select) => {
                    select.addEventListener('change', () => {
                        const index = Number(select.dataset.planScope);
                        state.plans[index].scope = select.value;

                        if (select.value !== 'variant') {
                            state.plans[index].variant_key = null;
                            state.plans[index].product_variant_id = null;
                        }
                ...
                roots.plans.querySelectorAll('[data-plan-variant]').forEach((select) => {
                    select.addEventListener('change', () => {
                        const index = Number(select.dataset.planVariant);
                        const variant = state.variants.find((entry) => entry.client_key === select.value);
                        state.plans[index].variant_key = select.value || null;
                        state.plans[index].product_variant_id = variant?.id || null;
                    });
                });

                roots.plans.querySelectorAll('[data-preview-plan]').forEach((button) => {
                    button.addEventListener('click', () => previewPlan(Number(button.dataset.previewPlan)));
                });

                roots.plans.querySelectorAll('[data-duplicate-plan]').forEach((button) => {
                    button.addEventListener('click', () => {
                        const source = state.plans[Number(button.dataset.duplicatePlan)];
                        state.plans.splice(Number(button.dataset.duplicatePlan) + 1, 0, {
                            ...source,
                            id: null,
                            is_active: false,
                        });
                ...
                roots.plans.querySelectorAll('[data-remove-plan]').forEach((button) => {
                    button.addEventListener('click', () => {
                        state.plans.splice(Number(button.dataset.removePlan), 1);
            ...
            const buildVariants = () => {
                ...
                roots.variants.innerHTML = `<div class="variant-table-wrap"><table class="variant-table"><thead><tr><th>Combination</th><th>SKU</th><th>Price</th><th>Compare At</th><th>Stock</th><th>Active</th><th>Advanced</th></tr></thead><tbody>${state.variants.map((variant, variantIndex) => `
                    <tr>
                        ...
                        <td data-label="Price"><input class="admin-input" type="number" step="0.01" min="0" name="variants[${variantIndex}][price]" value="${escapeHtml(variant.price || '')}" required></td>
            ...
            const syncPlanVariantTargets = () => {
                state.plans = state.plans.map((plan) => {
                    if (plan.scope !== 'variant') {
                        return { ...plan, variant_key: null, product_variant_id: null };
                    }

                    const matchedVariant = state.variants.find((variant) => variant.client_key === plan.variant_key)
                        || state.variants.find((variant) => String(variant.id || '') === String(plan.product_variant_id || ''));

                    return {
                        ...plan,
                        variant_key: matchedVariant?.client_key || plan.variant_key || null,
            ...
            const renderPreview = (plan, response) => {
                ...
                previewContext.textContent = `${headline(plan.interval_type)} schedule for ${plan.months} payments · ${planLabel}`;
                previewBody.innerHTML = `
                    <div class="admin-grid admin-grid-2">
                        ...
                        <div class="admin-card admin-card--tight">
                            <strong>Financing fee</strong>
                            <div>${money(preview.financing_fee)}</div>
                        </div>
                        <div class="admin-card admin-card--tight">
                            <strong>Down payment</strong>
                            <div>${money(preview.down_payment)}</div>
                        </div>
                        <div class="admin-card admin-card--tight">
            ...
            const previewPlan = async (index) => {
                ...
                const payload = {
                    ...
                    product_variant_id: plan.product_variant_id,
                    number_of_payments: Number(document.querySelector(`input[name="installment_plans[${index}][months]"]`)?.value || plan.months || 0),
                    down_payment: Number(document.querySelector(`input[name="installment_plans[${index}][down_payment]"]`)?.value || plan.down_payment || 0),
                    financing_fee: Number(document.querySelector(`input[name="installment_plans[${index}][financing_fee]"]`)?.value || plan.financing_fee || 0),
                    interval_type: document.querySelector(`select[name="installment_plans[${index}][interval_type]"]`)?.value || plan.interval_type || 'monthly',
                ...
                state.plans[index] = {
                    ...
                    financing_fee: payload.financing_fee,
