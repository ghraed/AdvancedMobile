...
(() => {
 ...
 purchase.onclick=async()=>{if(!payload?.in_stock||!selectedPlan)return;purchase.disabled=true;try{preview=await request(data.previewUrl,{variant_id:payload.variant_id,plan_id:selectedPlan.id});$('[data-modal-product]').textContent=`${preview.product} · ${preview.storage||'No storage'} · ${preview.color||'No color'}`;$('[data-modal-summary]').innerHTML=`<div>Variant price: <strong>${money(preview.variant_price)}</strong></div><div>First payment today: <strong>${money(preview.amount_due_now)}</strong></div><div>Each payment: <strong>${money(preview.installment_amount)}</strong></div><div>Number of payments: <strong>${preview.number_of_payments}</strong></div><div>Total amount: <strong>${money(preview.total_amount)}</strong></div>`;$('[data-modal-schedule]').innerHTML=preview.future_installments.map(p=>`<li class="rounded-xl border border-[var(--pm-border)] p-3">Payment ${p.sequence + 1} · <strong>${p.due_date}</strong> · ${money(p.amount)}</li>`).join('');$('[data-modal-status]').textContent='';modal.hidden=false;}catch(error){$('[data-status]').textContent=error.message;}finally{paymentState();}};
