...
(() => {
 ...
 function plans(){const section=$('[data-plan-section]'), list=$('[data-plan-options]'), calendar=$('[data-calendar]'); if(!payload?.plans?.length){section.hidden=true;selectedPlan=null;paymentState();return;} section.hidden=false; selectedPlan=payload.plans.find(p=>p.id===selectedPlan?.id)||null; list.innerHTML=payload.plans.map(p=>`<button type="button" data-plan="${p.id}" aria-pressed="${p.id===selectedPlan?.id}" class="rounded-2xl border px-3 py-3 text-left ${p.id===selectedPlan?.id?'border-white bg-white text-[var(--pm-primary)]':'border-white/20 bg-white/10'}"><strong>${p.payments} total payments</strong><span class="mt-1 block text-xs">Pay today: ${money(p.amount_due_now)}</span><span class="block text-xs">Remaining: ${p.future_payment_count} ${p.interval} payments</span><span class="block text-xs">Each future payment: ${money(p.installment_amount)}</span>${Number(p.financing_fee)?`<span class="block text-xs">Financing fee: ${money(p.financing_fee)}</span>`:''}<span class="block text-xs font-bold">Total financed amount: ${money(p.total)}</span></button>`).join(''); list.querySelectorAll('[data-plan]').forEach(b=>b.onclick=()=>{selectedPlan=payload.plans.find(p=>p.id===Number(b.dataset.plan));plans();}); if(!selectedPlan){calendar.innerHTML='Select a plan to see its payment timeline.';paymentState();return;} const segments=[`<li class="flex-1 rounded-lg bg-white px-2 py-2 text-center text-[11px] text-[var(--pm-primary)]"><strong>Today</strong><br>Due now<br>${money(selectedPlan.amount_due_now)}</li>`,...selectedPlan.schedule.map(p=>`<li class="flex-1 rounded-lg bg-white/15 px-2 py-2 text-center text-[11px]"><strong>Payment ${p.sequence + 1}</strong><br>${p.due_date}<br>${money(p.amount)}</li>`)].join(''); calendar.innerHTML=`<strong>Payment timeline: ${selectedPlan.payments} total payments</strong><p class="mt-1 text-xs">First payment is due today; the remaining payments are due on the dates shown.</p><ol class="mt-3 flex gap-1 overflow-x-auto">${segments}</ol>`;paymentState();}
 ...
 purchase.onclick=async()=>{if(!payload?.in_stock||!selectedPlan)return;purchase.disabled=true;try{preview=await request(data.previewUrl,{variant_id:payload.variant_id,plan_id:selectedPlan.id});$('[data-modal-product]').textContent=`${preview.product} · ${preview.storage||'No storage'} · ${preview.color||'No color'}`;$('[data-modal-summary]').innerHTML=`<div>Variant price: <strong>${money(preview.variant_price)}</strong></div><div>Payment due today: <strong>${money(preview.amount_due_now)}</strong></div><div>Future payment amount: <strong>${money(preview.installment_amount)}</strong></div><div>Number of future payments: <strong>${preview.future_payment_count}</strong></div><div>Financing fee: <strong>${money(preview.financing_fee)}</strong></div><div>Total amount: <strong>${money(preview.total_amount)}</strong></div>`;$('[data-modal-schedule]').innerHTML=preview.future_installments.map(p=>`<li class="rounded-xl border border-[var(--pm-border)] p-3">Payment ${p.sequence + 1} · <strong>${p.due_date}</strong> · ${money(p.amount)}</li>`).join('');$('[data-modal-status]').textContent='';modal.hidden=false;}catch(error){$('[data-status]').textContent=error.message;}finally{paymentState();}};
 const closeModal=()=>{modal.hidden=true;purchase.focus();}; $('[data-close-modal]').onclick=closeModal; document.addEventListener('keydown',event=>{if(event.key==='Escape'&&!modal.hidden)closeModal();}); $('[data-confirm-purchase]').onclick=async()=>{if(!preview)return;const confirm=$('[data-confirm-purchase]'),status=$('[data-modal-status]');confirm.disabled=true;confirm.textContent='Saving your selection…';try{const result=await request(data.confirmUrl,{variant_id:preview.variant_id,plan_id:preview.plan_id,return_url:window.location.pathname});status.className='mt-4 text-sm text-emerald-700';status.textContent=result.message;if(result.auth_url||result.checkout_url)window.location.assign(result.auth_url||result.checkout_url);}catch(error){status.className='mt-4 text-sm text-[var(--pm-danger)]';status.textContent=error.message;modal.hidden=false;resolve();}finally{confirm.disabled=false;confirm.textContent='Confirm installment selection';}};
